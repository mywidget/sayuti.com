                                    ...
                                    <table id="user" class="table table-bordered table-striped">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>email</th>
                                                <th>Tanggal</th>
                                                <th style="width:7%;text-align:center">Baca</th>
                                                <th style="width:5%;text-align:center">Aksi</th>
                                            </tr>
                                        </thead>
									<tbody>
<?php
$sql = $db->query("SELECT * FROM kotak_masuk order by id DESC");
$nom=1;
...
<tr>
	<td><?php echo $nom;?></td>
	<td><a href="?<?=$mode;?>=<?=$module;?>&act=baca&id=<?=$row['id'];?>"><?php echo $row['judul'];?></a></td>
...
// $y = 1;
// $edit = $db->prepare("UPDATE `kotak_masuk` SET `status`=? WHERE id=?");
// $edit->bind_param("ii",$y,$GETID);
// if($edit->execute())
// {
...
if($edit->execute())
{
	...
	if ($status=='unpublish'){
$y = 0;
$edit = $db->prepare("UPDATE `kotak_masuk` SET `baca`=? WHERE id=?");
