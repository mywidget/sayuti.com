...
$data = mysql_fetch_array(mysql_query("SELECT * FROM menuadmin WHERE idmenu=".$id));
...
?>
         ...
         <?php 
		 $tampil=mysql_query("SELECT * FROM menuadmin where idparent='0'");
          if ($data['idparent']==0){
          ...
          while($w=mysql_fetch_array($tampil)){
            if ($data['idparent']==$w['idmenu']){
              echo "<option value=$w[idmenu] selected>$w[nama_menu]</option>";
				...
				$subkategori=mysql_query("SELECT * from menuadmin WHERE idparent=$w[idmenu]");
				if ($data['idmenu']==0){
				//echo "<option value=0 selected>- Pilih kategori -</option>";
				} 
				while($wx=mysql_fetch_array($subkategori)){
...
$resultz = mysql_query("SELECT * FROM hak_akses");
while ($rowz = mysql_fetch_assoc($resultz)) {
$dataTz[$rowz['id_parent']][] = $rowz;
}
echo checkbox($dataTz,0,$_POST['id_parent'],$data['id_level']);
?>
                                                </div>
                </div><!-- /.box-body -->
										<!--div class="form-group">
