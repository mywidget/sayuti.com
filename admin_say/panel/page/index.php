                        ...
                        <div class="col-xs-12">
                            <div class="box">
                                ...
                                <div class="box-body">
									...
									<tbody>
<?php
$sql = $db->query("SELECT * FROM page order by id_page DESC");
$nom=1;
while($row=$sql->fetch_array()){
	if($row['pub'] == 0){
	$titles	= "unpublish";
	$links 	= "?$mode=$module&act=publish&status=$titles&id=$row[id_page]";
...
<tr>
	<td><?php echo $nom;?></td>
	<td><a href="?<?=$mode;?>=<?=$module;?>&act=edit&id=<?=$row['id_page'];?>"><?php echo $row['judul'];?></a></td>
	<td class="text-center"><?php echo $gbrs;?></td>
	<td><a href='?<?=$mode;?>=<?=$module;?>&act=edit&id=<?=$row['id_page'];?>'  data-toggle='tooltip' title='Edit Data'><i class='fa fa-edit'></i> </a> <a data-href="?<?=$mode;?>=<?=$module;?>&act=hapus&id=<?=$row['id_page'];?>" data-toggle="modal" data-target="#confirm-delete" href="#" data-toggle="tooltip" title="Hapus Data"><i class="fa fa-trash-o"></i></a></td>
</tr>
<?php $nom++; }	?>
									</tbody>
                                    </table>
...
<?php 
	...
	if ($status=='publish'){
$y = 'Y';
$edit = $mysqli->prepare("UPDATE `page` SET `publish`=? WHERE id_page=?");
