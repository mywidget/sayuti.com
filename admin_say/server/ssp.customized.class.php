    ...
    static function data_output ( $columns, $data, $isJoin = false )
    {
        ...
        for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
            ...
            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				elseif ( isset( $column['edit'] ) )
				{

				$id = ($isJoin) ? $column['edit']( $data[$i][ $column['field'] ], $data[$i] ) : $column['edit']( $data[$i][ $column['db'] ], $data[$i] );
							
				$row[ $column['dt'] ] = "<a href='read.php?panel=invoice&act=invoice&inv=".$id."&status=edit' data-toggle='tooltip'title='Rubah Invoice' class='btn btn-danger' style='height:22px;padding: 0px 12px;'>Modify</a>";
				
				$row[ $column['dt'] ] = '<a href="#" data-toggle="modal" data-target="#formadmin"  data-toggle="tooltip"  onclick="getid('.$i.')" title="Rubah Invoice"><i class="fa fa-edit"></i> </a> ';
				}
                else {
