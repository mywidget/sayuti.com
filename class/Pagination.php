...
class Pagination{
	...
	var $numLinks		=  3;
	var $currentPage	=  0;
	var $firstLink   	= '<i class="fa fa-chevron-left" aria-hidden="true"></i> First';
	var $nextLink		= '<i class="fa fa-chevron-right" aria-hidden="true"></i>';
	var $prevLink		= '<i class="fa fa-chevron-left" aria-hidden="true"></i>';
	var $lastLink		= 'Last <i class="fa fa-chevron-right" aria-hidden="true"></i>';
    ...
    var $additionalParam= '';
	var $link_func      = '';
	var $link_info_l	= '<li class="disabled"><a href="#">';
	...
	function createLinks(){ 
		...
		if ($numPages == 1){
			if ($this->showCount){
				$info = 'Menampilkan : ' . $this->totalRows;
				return $info;
			}else{
				return '';
			}
		...
		if ($this->showCount){
		   ...
		   $info = $this->link_info_l .'Menampilkan ' . ( $currentOffset + 1 ) . ' s.d ' ;
		   ...
		   $info .= ' dari ' . $this->totalRows . $this->link_info_r;
	...
	function getAJAXlink( $count, $text) {
        if($this->link_func == '' && $this->contentDiv == '')
            return $this->lil. ' <a href="'.$this->baseURL.'?'.$count.'"'.$this->anchorClass.'>'.$text.'</a>'. $this->lir;
	    ...
	    return $this->lil.  "<a href=\"javascript:void(0);\" " . $this->anchorClass . "
				". $linkClick .">". $text .'</a>'. $this->lir;
	}
}
?>